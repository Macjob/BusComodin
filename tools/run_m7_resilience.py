...
def summarize(rows):
    ...
    for (perturbation, policy), subset in sorted(grouped.items()):
        ...
        censored_waits = [float(r["mean_censored_wait_minutes"]) for r in subset]
        ...
        service_rates = [float(r["service_rate_pct"]) for r in subset]
    ...
    for policies in by_perturbation.values():
        ...
        baseline_censored = policies["baseline"]["mean_censored_wait_minutes"]
        for item in policies.values():
            item["wait_improvement_vs_baseline_pct"] = round((baseline - item["mean_wait_minutes"]) / baseline * 100, 2)
            item["censored_wait_improvement_vs_baseline_pct"] = round(
                (baseline_censored - item["mean_censored_wait_minutes"]) / baseline_censored * 100,
                2,
            )
    return summary
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run M7 resilience experiments")
    parser.add_argument("--scenario", default="configs/scenario-villa-alemana-real-am-v1.json")
    parser.add_argument("--output-dir", default="outputs/m7-resilience")
